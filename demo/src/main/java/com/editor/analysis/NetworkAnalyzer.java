...
public class NetworkAnalyzer {
    ...
    /*      
        ...
        public UserNode mostActive(SocialNetwork network)
            user with largest degree (followers + followings) or just adjacency size.

        public List<UserNode> mutualFollowers(SocialNetwork network, int[] ids)
            intersect follower sets (we can do intersection via lists and manual loops).

        public List<UserNode> suggestUsersToFollow(SocialNetwork network, int id)
            “followers of my followers” excluding already-followed users and myself.
